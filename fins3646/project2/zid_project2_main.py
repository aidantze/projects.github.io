...
def get_avg(df: pd.DataFrame, year):
    ...
    """
    # df = pd.DataFrame({'tic1': [-1, 1, 0, 1], 'tic2': [np.NaN, 0.032, -0.02, -0.02]},
    #                   index=['1999-10-13', '1999-10-14', '2010-10-15', '2010-10-16'])
...
def t_stat(EW_LS_pf_df):
    """
    Performs an event study on the equally weighted Long-Short Portfolio passed
    in as a dataframe. This involves the following:
    1. Extract data
    2. Get ls column. ls represents long-short portfolio returns, which is
    equivalent to Cumulative Abnormal Returns (CAR)
    3. Get number of observations, save to n_obs
    4. Calculate average ls, save to ls_bar
    5. Calculate variance and standard deviation (std)
    6. Calculate t-stat using the formula: ls_bar / (std / sqrt(n_obs)),
    and save to ls_t
    7. Construct a new dataframe with the following column names:
    - ls_bar
    - ls_t
    - n_obs
    ...
    ----------
    EW_LS_pf_df
        A dataframe of the equally weighted long-short portfolio generated
        from portfolio_main, which holds ewp_rank columns up to number of
        quartiles analysed, and ls representing the long-short portfolio
        returns, similar in calculation to CAR (i.e. difference between
        last ewp_rank and first ewp_rank)

    Returns
    -------
    df
        A dataframe consisting of the following columns:
        - ls_bar: mean of ls column from input dataframe
        - ls_t: t-stat of ls column from input dataframe
        - n_obs: number of observations of ls column from input dataframe


    """
    n_obs = get_n_obs(EW_LS_pf_df)
    ls_bar = get_ls_bar(EW_LS_pf_df, n_obs)
    ls_var = get_ls_var(EW_LS_pf_df, ls_bar, n_obs)
    ls_t = get_ls_t_stat(ls_bar, ls_var, n_obs)
    df = pd.DataFrame({'ls_bar': [ls_bar], 'ls_t': [ls_t], 'n_obs': [n_obs]})
    print(df)
    return df


def get_n_obs(EW_LS_pf_df):
    """
    Counts the number of observations for the given dataframe.

    Parameters
    ----------
    EW_LS_pf_df
        The dataframe containing values to count

    Returns
    -------
    n_obs: int
        The number of observations for the given dataframe.

    """
    return len(EW_LS_pf_df)



def get_ls_bar(EW_LS_pf_df, n_obs):
    """
    Computes the average of ls values for a given dataframe using the formula:
    ls_bar = 1/N * sum(ls)

    Parameters
    ----------
    EW_LS_pf_df
        The dataframe containing values to average

    Returns
    -------
    ls_bar: float
        The mean of ls values for a given dataframe

    """
    sum_total = EW_LS_pf_df['ls'].sum()
    average = (1 / n_obs) * sum_total
    return average


def get_ls_var(EW_LS_pf_df, ls_bar, n_obs):
    """
    Calculates the variance of ls values in EW_LS_pd_df using the formula
    ls_var = 1/(N-1) * sum((ls - ls_bar)**2)
    where ** means "...to the power of..."

    Parameters
    ----------
    EW_LS_pf_df
        The dataframe containing values to use in the variance formula
    ls_bar
        The mean of ls values for EW_LS_pf_df
    n_obs
        The number of observations in EW_LS_pf_df

    Returns
    -------
    ls_var: float
        The variance of ls values in EW_LS_pf_df

    """
    sum_square_diff = 0.0
    for entry in EW_LS_pf_df['ls']:
        square_diff = (entry - ls_bar)**2
        sum_square_diff += square_diff
    return (1 / (n_obs - 1)) * sum_square_diff


def get_ls_t_stat(ls_bar, ls_var, n_obs):
    """
    Calculates the t-statistic for ls values in EW_LS_pd_df
    ...
    ls_bar
        The mean of ls values for EW_LS_pf_df
    ls_var
        The variance of ls values for EW_LS_pf_df
    n_obs
        The number of observations in EW_LS_pf_df
    ...
    -------
    t_stat: float
        The t-statistic for the ls values in EW_LS_pf_df

    """
    return ls_bar/((ls_var**0.5)/(n_obs**0.5))
...
Presentation_zoom_link = 'https://unsw-my.sharepoint.com/:v:/g/personal/z5359105_ad_unsw_edu_au/ERCNd1qq2oRHrizKhqtqkRQB4vlqLjNZdhftdt6ACJQgng'
...
if __name__ == "__main__":
    # file_path = os.path.join(cfg.DATADIR, 'EW_LS_pf_df.csv')
