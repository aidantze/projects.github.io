...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    # Get the returns with the frequency given by ret_freq_use
    daily_returns = ret[ret_freq_use[0]].copy()

    # Resample the daily returns to monthly frequency
    # and set value as True if there is < 18 daily returns per month
    lt_18 = daily_returns.resample("ME").count().lt(18)

    # Get the volatility only for months that have >= 18 daily returns
    volatility = daily_returns.resample("ME").std()[~lt_18]

    # Rename df index
    volatility.rename_axis("Year_Month", inplace=True)

    # Rename df row labels with suffix that indicates the cha_name
    volatility = volatility.add_suffix(f"_{cha_name}")

    # Convert DatetimeIndex to monthly frequency PeriodIndex
    volatility = volatility.to_period("M")
...
def merge_tables(ret, df_cha, cha_name):
    ...
    # copy df and shift all cha values by 1 month
    df_cha_copy = df_cha.copy()
    df_cha_copy = df_cha_copy.shift(periods=1, axis=0)

    ret_copy = ret['Monthly'].copy()
    df = pd.merge(ret_copy, df_cha_copy, how='left', left_index=True, right_index=True)
    return df
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    merge_df = merge_tables(ret, vol_df, cha_name)
    return merge_df
