...
def read_prc_csv(tic, start, end, prc_col='Adj Close'):
    ...
    """
    PRC_CSV = os.path.join(cfg.DATADIR, f"{tic.lower()}_prc.csv")

    # <COMPLETE THIS PART>
    # Read csv in and set index to DatetimeIndex, sort dates
    prc = pd.read_csv(PRC_CSV, parse_dates=["Date"], index_col="Date")
    prc.sort_index(inplace=True)

    # Get required column, drop null entries, rename column to lowercase 'tic'
    ser = prc.loc[start:end].loc[:, prc_col].dropna()
    ser.rename(tic.lower(), inplace=True)
...
def daily_return_cal(prc):
    ...
    """
    # <COMPLETE THIS PART>
    # Get the daily prices and calculate the daily return
    daily_return = prc.pct_change()
...
def monthly_return_cal(prc):
    ...
    # <COMPLETE THIS PART>
    # This line:
    # 1. Changes daily frequency to month using resample("1ME")
    # 2. Checks number of entries resampled in a month
    count_ser = prc.resample("1ME").count()

    # Get end of month return and calculate monthly return
    eom_return = prc.resample("1ME").last()
    monthly_return = eom_return.pct_change()

    # Replace monthly return with NaN if number of price entries < 18
    monthly_return.mask(count_ser < 18, np.nan, inplace=True)

    # Change DatetimeIndex to PeriodIndex, rename series index
    monthly_return = monthly_return.to_period()
    monthly_return.rename_axis("Year_Month", inplace=True)
...
def aj_ret_dict(tickers, start, end):
    ...
    for ticker in tickers:
        data = read_prc_csv(ticker, start, end)

        daily_return = daily_return_cal(data)
        monthly_return = monthly_return_cal(data)

        # Concatenating
        daily_return_df = pd.concat([daily_return_df, daily_return], axis=1)
        monthly_return_df = pd.concat([monthly_return_df, monthly_return], axis=1)

    # Change to a datetime format and give it a name "Date"
    daily_return_df.index = pd.to_datetime(daily_return_df.index)
    daily_return_df.index.name = "Date"

    # Change to a monthly PeriodIndex format and give it a name "Year_Month"
    monthly_return_df.index = pd.PeriodIndex(monthly_return_df.index, freq="M")
